 ...
 */
public class OneToManyEdgeFactory implements EdgeFactoryInterface {

    @Override
    public String getCreateQuery(ObjectGraphDb db, Class edgeClass, Object from, Object to) throws Exception {
        return "Create Edge " + edgeClass.getSimpleName() + " FROM " + Utils.escapeRecordId(from,db) + " TO (select from " + Utils.escapeRecordId(to,db) + " where inE('" + edgeClass.getSimpleName() + "').size() < 1)" + " RETRY " + Config.getTRANSACTION_RETRY_COUNT();
    }
